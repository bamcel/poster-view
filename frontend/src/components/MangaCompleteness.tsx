...
type Saved = { id?: number; total?: number; finished?: boolean; owned?: string };
export default function MangaCompleteness({ serverId, item }: { serverId: number; item: ItemDetail }) {
  ...
  const expected = saved.total ?? (series?.status === "FINISHED" ? series.volumes : undefined);
  ...
  const label = !safeTotal || !ended || uncertain || matched.isError && !saved.total ? "Unknown" : missing.length ? "Missing volumes" : "Complete";
  ...
  return <section className="mt-8 rounded-xl border border-border bg-surface/95 p-4">
    ...
    <div role="status" aria-live="polite" className="mt-3 break-words">
      ...
      {!ended && <p className="mt-1 text-sm text-faint">A confirmed finished series and total are needed to determine completeness.</p>}
    ...
    {lookup.data && <div className="mt-2 space-y-2">
      ...
      {lookup.data.filter(entry => entry.format === "MANGA" || entry.format === "ONE_SHOT").map(entry => <button key={entry.id} type="button" className={`${buttonClass} w-full break-words text-left`} onClick={() => { persist({ ...saved, id: entry.id, total: undefined, finished: undefined }); setTotal(""); setFinished(false); }}>
    ...
    <details className="mt-4">
      ...
      <form className="mt-3 space-y-3" onSubmit={event => {
        ...
        const count = total.trim() ? Number(total) : undefined;
        if (count !== undefined && (!Number.isInteger(count) || count < 1 || count > 2000)) { setError("Expected volumes must be between 1 and 2000."); return; }
        ...
        persist({ id: saved.id, total: count, finished: count ? finished : undefined, owned: manual ? owned : undefined });
      }}>
        <label className="block text-xs">Expected volumes (leave blank to use AniList)<input type="number" min={1} max={2000} className={`mt-1 ${inputClass}`} value={total} onChange={e => setTotal(e.target.value)} /></label>
        ...
        <button type="button" className={`ml-2 ${buttonClass}`} onClick={() => { persist({}); setTotal(""); setFinished(false); setOwned(""); setManual(false); }}>Reset</button>
      </form>
    </details>
