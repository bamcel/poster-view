...
use posterview_contracts::{
    ApplyHistoryEntry, ApplyRequest, ApplyResult, ArtworkCacheClearResult, ArtworkCacheSettings,
    ArtworkCacheStatus, ArtworkProviderInfo, ArtworkProviderTestRequest, ArtworkProviderTestResult,
    ArtworkRefreshResult, ArtworkResults, ArtworkSearchResults, ArtworkSettings,
    ArtworkSettingsUpdate, ConnectionTest, HealthResponse, HistorySettings, ImageTarget,
    ItemDetail, Library, MediaItem, PosterDbCredentials, PosterDbStatus, PosterSearchResults,
    PosterSet, Server, ServerCreate, ServerUpdate, StatusResponse,
};
use posterview_infra_artwork::{ArtworkService, download_public_image, fetch_mediux_thumb};
use posterview_infra_media_servers::{
    ConnectionConfig, fetch_image, get_item_detail, get_items, get_libraries, set_image,
    test_connection,
};
use posterview_infra_sqlite::{HistoryRow, ServerStore, StoreError};
use thiserror::Error;
...
impl Runtime {
    ...
    pub async fn apply_download(
        ...
        let downloaded = if input.provider == "posterdb" {
            ...
            download_public_image(&input.download_url).await
        };
    ...
    fn server_store(&self) -> Result<&ServerStore, RuntimeError> {
        self.servers.get().ok_or(RuntimeError::NotInitialized)
    }
}

fn parse_nonnegative(value: &str, fallback: i64) -> i64 {
